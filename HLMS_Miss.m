function HLMSV3_Miss(sensor,feature,maxerr,mu1)
    %clc
    close all
    Nsamples=100000;
    fprintf('%d-----------------------------\n',sensor)
    %############################# Settings #########################################
     feature=4;                          % 3 = Temperature , 4= Humidity , 6=Infrared
     maxerr=0.1;                         % Maximum error treshold
    %#################################################################################
    arraysent=zeros(1,1);
    arrayperc=zeros(1,1);
    arrayMiss=zeros(1,1);
    arrayerror=zeros(1,10);
...
for GG=1:1   
    for G=sensor:sensor
        ...
        while n < Nsamples+1 
            ...
            if indexes(k) ~= n   %if in the index file at n there is a transmission

                euc=euc+power(abs(x(n)-y),2);
                errc = errc+1;

                %Update the filter on the sensor
                ...
                %if we are not in a missing phase
                if flag==0
                    %Miss or no ?
                    R = binornd(1,0.5); %0.1=10%
                    % how many succesive misses
                    c = randi([0 0]);
                else
                    r=0;
                    c=c-1;
                ...
                if R=1 %Miss
                    ...
                    %Missed transmission update Sink filter according to the
                    %prediction outputed.
% 
                    fprintf(fresults,'%d\n',y);
                    fprintf(Transmitted,'%s\n','nan');
                    ...
                    %succesfull transmission update the Sinks filter using the
                    %real recieved value.
        ...
        arraysent(GG)=sent;
        arrayperc(GG)=(100*sent)/Nsamples;
        arrayMiss(GG)=Miss;
